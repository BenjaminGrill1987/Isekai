...
public class AssigmentMenu : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        foreach (var entry in AssigmentSystem.GetAssigmentList())
        {
            ...
            panel.GetComponent<AssigmentPanel>().SetPanelText(entry.Name, entry.DescriptionText, entry.AssigmentReward, entry.AssigmentValue, entry.GetItemData().GetId(), entry.Type);
        }
